...
findIt 是一款面向家庭和个人物品管理的 Android App。它的核心亮点是把用户的自然语言描述转成可检索的数据结构：你可以用“现在厨房的顶柜里有沙拉酱和番茄酱”这样的表达记录物品，配合大模型自动识别物品、位置和标签，再把结果导入 App，日后即可通过名称、标签、地点或相关描述快速找回。

项目目前已经支持手动录入、搜索、地点管理、AI JSON 批量整理、数据导出，以及可被语音助手或快捷方式调用的 URL 深链接接口。目标是让个人用户用更自然的方式整理生活物品，而不是被迫逐项填写复杂表单。
...
项目当前的 Gradle 与 Android Gradle Plugin 组合需要使用 JDK 21 编译。JDK 26 等过高版本可能导致 Gradle/AGP 初始化失败，报出类似 `tag 26.0.1` 或无法打开 Gradle class cache 的错误。

如果本机默认 Java 版本不是 21，建议在 Android Studio 中把 Gradle JDK 设置为 JDK 21，或在本机的 `~/.gradle/gradle.properties` 中配置自己的 JDK 21 路径：

```properties
org.gradle.java.home=/path/to/your/jdk-21/Contents/Home
```
...
如果只想临时指定 JDK 21，也可以执行：

```bash
JAVA_HOME="/path/to/your/jdk-21/Contents/Home" ./gradlew :app:assembleDebug
```
